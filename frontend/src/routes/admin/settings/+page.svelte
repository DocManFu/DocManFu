	...
	{:else}
			...
			>
				<div class="flex items-center gap-2">
					{#if testResult.success}
						...
						<span class="text-sm font-medium text-green-800 dark:text-green-300">{testResult.message}</span>
					{:else}
						<span class="i-lucide-x-circle text-red-600 dark:text-red-400"></span>
						<span class="text-sm font-medium text-red-800 dark:text-red-300">{testResult.message}</span>
					{/if}
					<button
		...
		<div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 space-y-6">
			...
			{#if provider !== 'none'}
				...
				{#if provider === 'openai'}
					<div class="rounded-lg border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 p-4 text-sm">
						...
						<ol class="list-decimal list-inside space-y-1 text-blue-800 dark:text-blue-300">
							<li>Go to <a href="https://platform.openai.com/api-keys" target="_blank" rel="noopener" class="underline font-medium hover:text-blue-600 dark:hover:text-blue-100">platform.openai.com/api-keys</a></li>
							<li>Sign in or create an account</li>
							<li>Click "Create new secret key" and copy it</li>
							<li>Paste the key below (starts with <code class="bg-blue-100 dark:bg-blue-800 px-1 rounded text-xs">sk-</code>)</li>
						</ol>
						<p class="mt-2 text-xs text-blue-600 dark:text-blue-400">
							You'll need a payment method on your OpenAI account. See
							<a href="https://platform.openai.com/docs/pricing" target="_blank" rel="noopener" class="underline hover:text-blue-500 dark:hover:text-blue-200">pricing details</a>.
						</p>
					</div>
				{:else if provider === 'anthropic'}
					<div class="rounded-lg border border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-900/20 p-4 text-sm">
						<h4 class="font-medium text-purple-900 dark:text-purple-200 mb-2 flex items-center gap-1.5">
						...
						<ol class="list-decimal list-inside space-y-1 text-purple-800 dark:text-purple-300">
							<li>Go to <a href="https://console.anthropic.com/settings/keys" target="_blank" rel="noopener" class="underline font-medium hover:text-purple-600 dark:hover:text-purple-100">console.anthropic.com/settings/keys</a></li>
							<li>Sign in or create an account</li>
							<li>Click "Create Key" and copy it</li>
							<li>Paste the key below (starts with <code class="bg-purple-100 dark:bg-purple-800 px-1 rounded text-xs">sk-ant-</code>)</li>
						</ol>
						<p class="mt-2 text-xs text-purple-600 dark:text-purple-400">
							You'll need to add credits to your account. See
							<a href="https://docs.anthropic.com/en/docs/about-claude/models" target="_blank" rel="noopener" class="underline hover:text-purple-500 dark:hover:text-purple-200">model details and pricing</a>.
						</p>
					</div>
				{/if}
				...
				{#if provider === 'openai' || provider === 'anthropic'}
					<div>
						...
						{#if !useCustomModel}
							...
							<!-- Model info card -->
							{@const displayModel = selectedModelInfo || providerModels.find((m) => m.id === (model || providerModels[0]?.id))}
							{#if displayModel}
								<div class="mt-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-3">
									...
									<p class="text-[10px] text-gray-400 dark:text-gray-500 mt-1">
										...
										{#if provider === 'openai'}
											<a href="https://platform.openai.com/docs/pricing" target="_blank" rel="noopener" class="underline">OpenAI pricing</a>
										{:else}
											<a href="https://docs.anthropic.com/en/docs/about-claude/models" target="_blank" rel="noopener" class="underline">Anthropic pricing</a>
										{/if}
										for current rates.
									</p>
								</div>
						...
						{:else}
							...
							<p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
								{#if provider === 'openai'}
									...
									<a href="https://platform.openai.com/docs/models" target="_blank" rel="noopener" class="underline hover:text-gray-700 dark:hover:text-gray-300">platform.openai.com/docs/models</a>
								{:else}
									See all available models at
									<a href="https://docs.anthropic.com/en/docs/about-claude/models" target="_blank" rel="noopener" class="underline hover:text-gray-700 dark:hover:text-gray-300">docs.anthropic.com/en/docs/about-claude/models</a>
								{/if}
							</p>
						{/if}
					</div>
				{:else}
					...
					<div>
						...
						/>
						<p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
							Leave empty for the default model. Run <code class="bg-gray-100 dark:bg-gray-700 px-1 rounded">ollama list</code> to see installed models.
						</p>
					</div>
				{/if}
				...
				{#if needsBaseUrl}
					<div>
						<div class="flex items-center gap-2 mb-1">
							<label for="base-url" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
								Base URL
							</label>
				...
				<div class="border-t border-gray-200 dark:border-gray-700 pt-4">
					...
					>
						<span class="{showAdvanced ? 'i-lucide-chevron-down' : 'i-lucide-chevron-right'} text-xs"></span>
						Advanced Settings
					</button>
					...
					{#if showAdvanced}
						<div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
							...
							<div>
								<div class="flex items-center gap-2 mb-1">
									<label for="max-text" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
										Max Text Length
									</label>
							...
							<div>
								<div class="flex items-center gap-2 mb-1">
									<label for="max-pages" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
										Max Vision Pages
									</label>
							...
							<div>
								<div class="flex items-center gap-2 mb-1">
									<label for="vision-dpi" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
										Vision DPI
									</label>
							...
							<div class="sm:col-span-2">
								<div class="flex items-center gap-2 mb-1">
									<label for="vision-model" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
...
{#snippet sourceBadge(source: string)}
	{#if source === 'database'}
		<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-brand-100 text-brand-700 dark:bg-brand-900/30 dark:text-brand-400">
			database
		</span>
	{:else if source === 'env'}
		<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-gray-200 text-gray-600 dark:bg-gray-700 dark:text-gray-400">
			env
		</span>
	{:else}
		<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-500">
			default
		</span>
	{/if}
{/snippet}
